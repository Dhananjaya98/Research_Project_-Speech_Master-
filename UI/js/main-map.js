function mainMap(locationParm, restBaseUrl) {


        var locations = [];

        function markerIcon(longitude, latitude) {
            var marker = {
                anchor: new google.maps.Point(longitude, latitude),
                url: 'images/marker.png',
                labelOrigin: new google.maps.Point(longitude, latitude)
            }
            return marker;
        }

        var centerLocation = {
            lng: locationParm[0].longitude,
            lat: locationParm[0].latitude
        };
        ...
        locationParm.forEach((element, index) => {
            var cover_img;
            if (element.cover_img) {
                cover_img = restBaseUrl + element.cover_img;
            } else if (element.hallImageDtos && element.hallImageDtos.length > 0) {
                console.log("truee")
                cover_img = restBaseUrl + element.hallImageDtos[0].imageUrl;
            } else {
                console.log("Falee")
                console.log(element.hallImageDtos)
                cover_img = "images/mapdefault.jpg"
            }
            locations.push([locationData("", cover_img, element.name, "", "", element.rating), element.latitude, element.longitude, index, markerIcon(element.longitude, element.latitude)]);
        });
        //  Map Infoboxes ------------------
        //  var locations = [
